 ...
 */
Samotraces.UI.Widgets.ObselTypeInspectorCocher = function(html_id,obsel_selector) {
...
Samotraces.UI.Widgets.ObselTypeInspectorCocher.prototype = {
	init_DOM: function() {
	...
	inspect: function(event) {
		...
		//li_element.appendChild(document.createTextNode('type: '+ obs.get_type()));
         ...
         chek.setAttribute('type','checkbox');
          chek.setAttribute('class','checkboxType');
         chek.setAttribute('name',obs["type"]);
         if (obs.coche)
         {chek.checked = true}
         li_element.appendChild(chek);
		...
		for(var key in obs.attributes) {
			   ...
			   if (val=="@id")
		            ...
		            chek = document.createElement ("input");
                    chek.setAttribute('type','checkbox');
                    chek.setAttribute('class','checkboxAtt');
                    chek.setAttribute('name',obs.attributes[key][val]);
                    if (obs.attributes[key].coche)
                        {chek.checked = true}
